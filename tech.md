...
**Версия ядра: v11** · 2026-08-15

## Changelog

- **v11** — страница доставки открывается полосой из четырёх колонок: магазин, режим работы, самовывоз, доставка. Раздел 4.7 получил `pickup_text_uk/ru` и `delivery_text_uk/ru`, раздел 12 — примитив `info_columns.html`, раздел 10 — абзац об этой полосе. Причина: адрес и часы в настройках уже были, а текстов о самовывозе и доставке нет, и собрать колонки в теле `StaticPage` нельзя — bleach оставляет из атрибутов только `href`, `src`, `alt`, `width`, `height`, классы вырезаются.
- **v10** — раздел 13 разделил анимации на два вида. 150–250 мс относятся к реакции на действие: наведение, нажатие, появление панели. Анимации ожидания и подтверждения живут по своим правилам: мерцание скелетона идёт циклом 1.4 с, отклик сердечка избранного — 320 мс. Причина: прежняя формулировка запрещала и то и другое, хотя скелетоны уже мерцали через `animate-pulse` на 2 с. Кадры лежат в `static/css/input.css`; блок `prefers-reduced-motion` там же гасит и их.
...
- **Доставка:** `pickup_text_uk/ru` (CharField 300, blank), `delivery_text_uk/ru` (CharField 300, blank) — две колонки полосы на `/dostavka-i-oplata/`
...
Страница `/dostavka-i-oplata/` открывается полосой `info_columns.html` из четырёх колонок: адрес магазина, режим работы, самовывоз, доставка. Первые две берутся из `address` и `working_hours`, вторые две — из `pickup_text` и `delivery_text` раздела 4.7. Колонка без текста не рисуется. Ниже идёт обычное тело `StaticPage`.
...
| `info_columns.html` | `columns` — список словарей `{icon, title, text}`. Ряд колонок с иконкой из спрайта, подписью и текстом под ней. 1 колонка на мобильном, 2 от `sm`, до четырёх в ряд от `lg` |
...
- **Анимации.** Реакция на действие — 150–250 мс, `ease-out`: наведение, нажатие, выезд панели, смена фрагмента HTMX. Ожидание и подтверждение — отдельно: мерцание скелетона 1.4 с циклом, отклик сердечка 320 мс. И то и другое обязано гаситься при `prefers-reduced-motion`; блок в `input.css` снимает длительности глобально.
