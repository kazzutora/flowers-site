...
1. **Hero** — веер фотографий и текст поверх него: заголовок, подзаголовок, кнопки «Дивитись галерею» и «Зателефонувати». Рисуется примитивом `hero_showcase.html`. Ближняя карточка веера — `SiteSettings.hero_image`, за ней до двух обложек поводов; ближняя грузится с `fetchpriority="high"`, остальные ленивые. На мобильном веер схлопывается в одну карточку, текст встаёт над ней. Веер декоративен: он скрыт от скринридера, весь смысл несут заголовок и подзаголовок.
...
| `hero_showcase.html` | `images` (1–3 изображения, первое — ближняя карточка; лишние игнорируются), `title`, `subtitle`, `body_template` (шаблон с кнопками, рендерится под подзаголовком). Веер карточек в перспективе, текст поверх ближней. Ближняя картинка `priority`, остальные ленивые; на мобильном рисуется только ближняя. Веер несёт `aria-hidden` |
...
- **Сетка:** контейнер `max-w-site`, боковые отступы 16px на мобильном, 32px от `md`.
