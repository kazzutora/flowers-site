...
**Логотип.** `static/img/logo.webp` — локап с названием магазина, собранный из исходника `static/img/main.png` обрезкой прозрачных полей и уменьшением до 443×160. Рядом с ним название текстом не дублируется, оно живёт в `alt`. Знак сохраняет собственный `#A95DA0` и не перекрашивается в `accent`: это авторская работа, а не иконка.
